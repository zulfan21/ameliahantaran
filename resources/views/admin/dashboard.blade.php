...
@section('title', 'Dashboard Admin - Amelia Hantaran')
    ...
    <div class="space-y-6">
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-semibold">{{ $stats['total_products'] }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-gray-600">Stok Menipis</span>
                            <span class="font-semibold text-red-600">{{ $stats['low_stock_products'] }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
                    ...
                    <div class="space-y-3">
                        <a href="{{ route('admin.orders.index', ['status' => 'payment_verification']) }}"
                            ...
                            <span
                                class="px-2 py-1 bg-yellow-500 text-white text-xs rounded-full">{{ $stats['pending_payments'] }}</span>
                        </a>
                        <a href="{{ route('admin.testimonials.index', ['status' => 'pending']) }}"
                            ...
                            <span
                                class="px-2 py-1 bg-blue-500 text-white text-xs rounded-full">{{ $stats['pending_testimonials'] }}</span>
                        </a>
                    </div>
                </div>
            </div>
                            ...
                            @foreach ($lowStockProducts as $product)
                                <tr class="border-b border-gray-100 last:border-0">
                                    ...
                                    <td class="py-3 px-4 text-center">
                                        <span
                                            class="px-2 py-1 bg-red-100 text-red-700 rounded-full text-sm">{{ $product->stock }}</span>
                                    </td>
        ...
        <div class="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.orders.index') }}" class="text-sm text-primary-600 hover:text-primary-700">Lihat
                    Semua</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach ($recentOrders as $order)
                            <tr class="border-b border-gray-100 last:border-0">
                                ...
                                <td class="py-3 px-4 text-center">
                                    <span
                                        class="px-2 py-1 bg-{{ $order->status_color }}-100 text-{{ $order->status_color }}-700 rounded-full text-xs">
                                        {{ $order->status_label }}
                                    </span>
                                </td>
                                <td class="py-3 px-4 text-right">
