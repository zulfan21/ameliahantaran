...
<div class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 group">
    <!-- Image -->
    ...
    <!-- Content -->
    <div class="p-5">
        <span class="text-xs text-primary-600 font-medium uppercase tracking-wide"><?php echo e($product->category->name); ?></span>
        <h3 class="font-semibold text-lg text-gray-900 mt-1 mb-2 line-clamp-1"><?php echo e($product->name); ?></h3>
        <p class="text-gray-500 text-sm line-clamp-2 mb-3"><?php echo e(Str::limit($product->description, 80)); ?></p>
        
        <div class="flex items-center justify-between">
            <span class="text-primary-600 font-bold text-lg"><?php echo e($product->formattedPrice()); ?></span>
            
            <?php if($product->isInStock()): ?>
                <form action="<?php echo e(route('cart.add', $product->id)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                    <button type="submit" class="w-10 h-10 bg-primary-100 text-primary-600 rounded-lg flex items-center justify-center hover:bg-primary-500 hover:text-white transition-colors">
                        <i data-lucide="shopping-cart" class="w-5 h-5"></i>
                    </button>
                </form>
            <?php else: ?>
                <button disabled class="w-10 h-10 bg-gray-100 text-gray-400 rounded-lg flex items-center justify-center cursor-not-allowed">
                    <i data-lucide="shopping-cart" class="w-5 h-5"></i>
                </button>
            <?php endif; ?>
        </div>
